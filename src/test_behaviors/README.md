# FlexBE States and Behaviors for test
...
This raw repository has several folders and files with the generic name `test`.
...
and change the relevant `test` text to `my_new_project_name` as needed.
...
## Example States in `test_flexbe_states`
...
## Example Behaviors in `test_flexbe_behaviors`
...
[`copy_behavior`](test_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run test_flexbe_behavior copy_behavior` to see the usage guide. 
