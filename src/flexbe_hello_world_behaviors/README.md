# FlexBE States and Behaviors for flexbe_hello_world
...
This raw repository has several folders and files with the generic name `flexbe_hello_world`.
...
and change the relevant `flexbe_hello_world` text to `my_new_project_name` as needed.
...
## Example States in `flexbe_hello_world_flexbe_states`
...
## Example Behaviors in `flexbe_hello_world_flexbe_behaviors`
...
[`copy_behavior`](flexbe_hello_world_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run flexbe_hello_world_flexbe_behavior copy_behavior` to see the usage guide. 
